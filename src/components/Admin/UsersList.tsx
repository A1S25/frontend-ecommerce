...
const UsersList = ({ history }:any) => {
  const dispatch = useDispatch();
  //const alert = useAlert();
  const { error, users } = useSelector((state:any) => state.allUsers);
  const { error: deleteError, isDeleted, message, } = useSelector((state:any) => state.profile);

  const deleteUserHandler = (id:any) => { dispatch(deleteUser(id)); };
  ...
  const columns = [
    ...
    { field: "role", headerName: "Role", type: "number", minWidth: 150,  flex: 0.3,
      cellClassName: (params:any) => { return params.getValue(params.id, "role") === "admin" ? "greenColor" : "redColor"; },
    },
